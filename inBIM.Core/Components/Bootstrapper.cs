...
namespace inBIM.Core.Components
{
	 public sealed class Bootstrapper : UnityBootstrapper
	 {
		  ...
		  {
			   string LogsFolder = Path.Combine(Environment.GetEnvironmentVariable("AppData"), "inBIM", "Logs");
			   ...
			   for(int Count = 0; Count <= 100; Count++)
			   {
					try
					{
						 writer = File.CreateText(Path.Combine(LogsFolder, $"inBIM Log - {ProcessVersion.ProductName}({ProcessVersion.ProductMajorPart}_{ProcessVersion.ProductMinorPart}) Session({Count}).txt"));
						 break;
					}
					catch { }
			   }
