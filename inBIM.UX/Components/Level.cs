...
namespace SIG.Model
{
	 ...
	 internal class Level
	 {
		  ...
		  /// Constructor
		  /// </summary>
		  public Level(Size p_gameSize, Size p_pixelSize, Game game)
		  {
			   _game = game;
			...
			{
				{ _game._TextLevelStart, _game._PosTextLevelStart },
				{ _game._TextGameOver, _game._PosTextGameOver },
				{ _game._TextLevelCompleted, _game._PosTextLevelCompleted },
				{ _game._TextRestart, _game._PosTextRestart },
				{ _game._TextStartNewLevel, _game._PosTextStartNewLevel },
			};
		  ...
		  {
			   Bomb sprite = new Bomb(Constants.PATTERNBOMB, _game._BrushBomb, this.PixelSize, p_position, p_sequence);
		  ...
		  private Invader CreateInvader(Point p_position, int p_type, int[] p_sequence)
		  {
			   ...
			   {
					default:
					case 1: sprite = new Invader(Constants.PATTERNINVADER1, _game._BrushInvader1, this.PixelSize, p_position, p_sequence, 30); break;
					case 2: sprite = new Invader(Constants.PATTERNINVADER2, _game._BrushInvader2, this.PixelSize, p_position, p_sequence, 20); break;
					case 3: sprite = new Invader(Constants.PATTERNINVADER3, _game._BrushInvader3, this.PixelSize, p_position, p_sequence, 10); break;
			   }
		  ...
		  {
			   return new Missile(Constants.PATTERNMISSILE, _game._BrushMissile, this.PixelSize, p_position);
		  }
		  ...
		  {
			   Player sprite = new Player(Constants.PATTERNPLAYER, _game._BrushPlayer, this.PixelSize, p_position);
			   this.Player = sprite;
			   m_sprites.Add(sprite);
		  ...
		  {
			   Saucer sprite = new Saucer(Constants.PATTERNSAUCER, _game._BrushSaucer, this.PixelSize, p_position, new int[] { 0, 1 }, ( uint )m_randomNumber.Next(5, 15) * 10);
			   this.Saucer = sprite;
			   m_sprites.Add(sprite);
		  ...
		  {
			   Shield sprite = new Shield(Constants.PATTERNSHIELD, _game._BrushShield, this.PixelSize, p_position);
			   m_sprites.Add(sprite);
		  ...
		  private void DestroyEnemy(IEnemy p_enemy)
		  {
			   ...
			   if(p_enemy is Invader)
			   {
					...
					this.EnemyExplosion = new Explosion(Constants.PATTERNEXPLOSION, invader.Foreground, this.PixelSize, invader.GetPosition(), 20);
			   }
		  ...
		  public void Exit()
		  {
			   ...
			   if(this.Status == GameStatus.Exited)
			   {
					return;
			   }
			   ...
			   m_taskLoop = Task.Factory.StartNew(( Action )(() =>
				 {
					  ...
					  while((this.Status != GameStatus.Exiting) && (m_keyPressed != Key.Escape))
					  {
								...
								if(counter >= 300)
									 {
										  ...
										  {
											   SetDisplayedMiddleText(_game._TextStartNewLevel);
										  }
										  else
										  {
											   SetDisplayedMiddleText(_game._TextRestart);
										  }
									 }
						   ...
						   this.Player.BecomeInvincible();
						   SetDisplayedMiddleText(_game._TextLevelStart);
						   ...
						   if(this.Status == GameStatus.LevelCompleted)
						   {
								SetDisplayedMiddleText(_game._TextLevelCompleted);
						   }
						   else if(this.Status == GameStatus.Stopped)
						   {
								SetDisplayedMiddleText(_game._TextGameOver);
						   }
		  ...
		  private void Reset()
		  {
			   ...
			   m_playerHorizontalStep = this.PixelSize.Width * 0.5;
		  ...
		  private void MoveBombs()
		  {
			   ...
			   foreach(Bomb bomb in m_sprites.Where(s => s is Bomb))
			   {
					...
					if(position.Y > _game.Screen.Height)
					{
					...
					{
						 bomb.SetPosition(position.X, position.Y + m_bombVerticalStep);
					}
			   }
		  ...
		  private void MoveInvaders()
		  {
			   ...
			   foreach(Invader invader in invaders)
			   {
					...
					if(position.Y > 400)
					{
						 ...
						 this.PlayerExplosion = new Explosion(Constants.PATTERNEXPLOSION, this.Player.Foreground, this.PixelSize, this.Player.GetPosition(), 80);
						 DestroyPlayer();
